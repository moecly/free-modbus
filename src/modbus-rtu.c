...
static int _modbus_rtu_prepare_response_tid(const uint8_t *req,
                                            int *req_length) {
...
#if defined(_WIN32)
  return win32_ser_read(&((modbus_rtu_t *)ctx->backend_data)->w_ser, rsp,
                        rsp_length);
#else
...
static int _modbus_rtu_connect(modbus_t *ctx) {
   ...
   * number */
  ctx_rtu->w_ser.fd = CreateFileA(ctx_rtu->device, GENERIC_READ | GENERIC_WRITE,
